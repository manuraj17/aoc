...
if final_size > REQUIRED
  unused_space = (TOTAL_SPACE - final_size)
end

to_be_deleted = REQUIRED - unused_space
pp unused_space
pp to_be_deleted

def find_dir(n, unused_space, to_be_deleted, dirs)
  pp unused_space
  pp to_be_deleted
  pp "no children"
  if n.el_type == DIR
    pp "target: #{n.el_size + unused_space}"
    if (n.el_size) >= to_be_deleted
      pp "Pushing #{n.el_name}"
      dirs.push(n)
    end
  end
  ...
  n.el_children.each do |c|
    ...
    pp c.el_type == DIR
    if c.el_type == DIR
      dirs = find_dir(c, unused_space, to_be_deleted, dirs)
    end
  end
...
dirs = []
puts "fiding dirs"
dirs = find_dir(root, unused_space, to_be_deleted, dirs)
...
puts "Before"
pp dirs.count
pp dirs.first.el_size
pp dirs.last.el_size

dirs.sort_by!{ |e| e.el_size}
